...
- `merchant` 型別的 `match_value` 目前直接放**正規化後的原始商家名稱字串**，還沒有真的對應到 `merchants.merchant_id`。完整版需要先把 Allen 爬到的原始商家名稱一一比對出 `merchant_id`（近 900 筆，屬於之後要做的別名比對工作），這次時間有限先跳過，讓 evaluator 邏輯可以先用字串比對跑通。
- CUBE 卡「一般消費」的預設回饋率（0.3%）是沿用先前 mock 資料的假設值，Allen 的爬蟲資料裡目前沒有明確的「未命中任何方案時的基礎回饋」數字，需要另外確認官網條款。
- CUBE 卡的「台塑家」「全支付」兩個方案，`required_action` 先設為 `null`（假設不需要手動切換），這只是假設，還沒有跟官網條款或 Allen 核對過。
- 「慶生月」「童樂匯」兩個方案，依照你上次的決定先不轉換進來，等未來要擴充新方案時再依同樣模式加入。
